...
function cleanBranding(text) {
  ...
  // Clean copyright or ownership mentions
...
async function checkJobExists(title, accessToken) {
  ...
  if (!res.ok) {
    ...
    console.error('Query failed details:', errText);
...
// Helper to check if a last date has passed
function isExpired(lastDateStr) {
  if (!lastDateStr) return false;
  try {
    const cleanDate = lastDateStr.split(/[|\s(]/)[0].trim();
    const parts = cleanDate.split(/[\/\-]/);
    if (parts.length === 3) {
      const day = parseInt(parts[0]);
      const month = parseInt(parts[1]) - 1;
      const year = parseInt(parts[2]);
      const lDate = new Date(year, month, day, 23, 59, 59);
...
async function saveToFirestore(data, accessToken, existingDocId = null, existingData = null) {
  ...
  // Smart Badge logic: Only 'New' if posted within last 7 days
  const now = new Date();
  const diffDays = (now - createdAtDate) / (1000 * 60 * 60 * 24);
  if (data.category === 'Latest Job') {
    if (diffDays <= 7) {
      data.badge = 'New';
    } else {
      data.badge = 'Open';
...
function createProfessionalTable(data) {
  ...
  if (lastDate) datesList.push(`<li>Last Date Pay Exam Fee : <strong>${lastDate}</strong></li>`);
  ...
  // Map dynamic links with prioritized sorting: Apply Online first, then Admit Card, Syllabus, Notification, Official Website
  const linkPriority = {
    'apply online': 1,
...
function cleanCategoryTable(scrapedTable, commissionText) {
  ...
  if (isTitleRow) {
    ...
    headerRowsHTML += `<tr><th colspan="${colCount}" class="sarkari-cell-center">${commissionText ? commissionText + ' ' : ''}Category Wise Vacancy Details</th></tr>`;
  }
...
async function main() {
  ...
  try {
    ...
    anchors.forEach(a => {
      ...
      if (isJobUrl) {
        tasks.push({
          ...
          category: 'Latest Job',
          label: text,
          sortIndex: discoveredSortIndex++ // Preserve Sarkari Result order!
        });
      }
    });
    ...
    console.log(`Discovered ${tasks.length} potential jobs. Selected top ${finalTasks.length} for import.`);
    ...
    for (let i = 0; i < finalTasks.length; i++) {
      ...
      // Add Sort Index (maintain Sarkari Result serial order)
      scrapedData.sortIndex = task.sortIndex;

      if (isExpired(scrapedData.lastDate)) {
        console.log(`  [SKIP] Last date passed (${scrapedData.lastDate}). Skipping import...`);
        skippedCount++;
        continue;
      }
      ...
      // Apply Branding Cleaner to all fields
      ...
      // Ensure title is trimmed for database matching

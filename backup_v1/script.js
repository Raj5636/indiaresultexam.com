...
async function loadByCategoryToGrid(category, targetEl) {
  ...
  const handler = (snap) => {
    ...
    console.log('Category raw data:', snap.docs.map(d => ({ 
      id: d.id, 
      title: d.data().title,
...
function jobCardTemplate(job) {
  ...
  const whatsappButtonHtml = `<a class="btn-whatsapp" href="${whatsappLink}" target="_blank" rel="noopener noreferrer" title="${btnLabel}"><i class="fa-brands fa-whatsapp"></i> ${btnLabel}</a>`;
...
function filterJobs(state) {
  if (!jobsGrid) return;
  const queryText = getActiveSearchQuery();
  ...
  cards.forEach((card) => {
    ...
    card.style.display = (matchesState && matchesSearch) ? '' : 'none';
  });
}
...
function filterClassicCategory(category) {
  ...
  if (trio) {
    ...
    trio.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }
}
...
async function loadJobsFromFirestore() {
  ...
  try {
    ...
    unsubJobsList = onSnapshot(qMain, (snap) => {
      ...
      console.log('Raw snapshot data:', snap.docs.map(d => ({ id: d.id, data: d.data() })));
      console.log('Data structure check:', snap.docs.map(d => ({ 
        id: d.id, 
        title: d.data().title,
        category: d.data().category,
        createdAt: d.data().createdAt,
